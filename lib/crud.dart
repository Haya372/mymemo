...
class Crud {
  ...
  Future<List<Map>> selectAll() async {
    Database db = await database;
    List<Map> res = await db.rawQuery('select * from memos order by updated_at desc');
    return res;
  }

  Future<List<Map>> select(int tag) async {
    Database db = await database;
    List<Map> res = await db.rawQuery('select * from memos where tag = $tag order by updated_at desc');
    return res;
  }
